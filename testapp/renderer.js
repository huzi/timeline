'use strict';
var Renderer = function (timeline, selector, data) {
    var that = this;
...
Renderer.prototype._filterData = function () {
    var that = this;
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (dataTime > scaleStart - scaleDelta && dataTime < scaleEnd + scaleDelta) {
            var item = data[i];
            item.x = that.timeline.scale(item.date);
            item.type = 'single';
            var thresholdDate = that.timeline.scale.invert(item.x + 15)
            var cnt = i + 1;
            ...
            if (nextItem) {
                item.width = that.timeline.scale(nextItem.date) - item.x;
            }
            item.cnt = cnt - i;
            i = cnt - 1;
            filtered.push(item);
...
Renderer.prototype.draw = function () {
    ...
    var that = this;
    var offset = that.timeline.scale(that._filtered[0].date);
    ...
    that.dataSvgGraphic
        .attr('transform', function () {
            that._filtered.forEach(function (point) {
                //console.log(point.date, that.timeline.scale(point.date))
            });
    ...
    updateSelection.select('line')
        .attr('x1', function (data) {
            return that.timeline.scale(data.date) - offset;
    ...
    enterSelection.append('g')
        ...
        .call(function (g) {
            ...
            var text = g.append('text').text(_getText).attr('x', function (data) {
                return that.timeline.scale(data.date) + 5;
            }).attr('y', 10).each(function () {
...
Renderer.prototype.redraw = function () {
    ...
    that.dataSvgGraphic
        .attr('transform', function () {
            ...
            return 'translate(' + that.timeline.scale(that._filtered[0].date) + ')';
        });
    /*var updateSelection = that.dataSvgGraphic.selectAll('g.data-item').data(that._filtered);
    updateSelection.selectAll('line')
        .attr('x1', function (data) {
            return that.timeline.scale(data.date);
        }).attr('x2', function (data) {
            return that.timeline.scale(data.date);
        });
    updateSelection.selectAll('text').each(function () {
        var d3This = d3.select(this);
        var d3Data = d3This.data()[0];
        var textWidth = d3Data.width
        if (d3Data.x < 0 && d3Data.x + d3Data.width > 15) {
...
*/
    //console.log('renderer redraw', new Date().getTime() - start);
